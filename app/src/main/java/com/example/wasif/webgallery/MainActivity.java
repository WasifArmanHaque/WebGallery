...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ArrayList<WebImage> imageUrlList = loadData();

        RecyclerView rv = (RecyclerView) findViewById(R.id.card_recycler_view);
        rv.setHasFixedSize(true);

        GridLayoutManager glm = new GridLayoutManager(getApplicationContext(),4);
        rv.setLayoutManager(glm);

        DataAdapter adapter = new DataAdapter(getApplicationContext(), imageUrlList);
        rv.setAdapter(adapter);


    }

    private ArrayList<WebImage> loadData() {
        ArrayList<WebImage> imageUrlList = new ArrayList<WebImage>();

        int len = URL.imageUrls.length;
        for(int i = 0; i < len; i++)
        {
            WebImage webimg = new WebImage();
            webimg.setImage_url(URL.imageUrls[i]);
            webimg.setImage_thumb_url(URL.imageThumbUrls[i]);
            imageUrlList.add(webimg);
        }
        return imageUrlList;
    }
}
